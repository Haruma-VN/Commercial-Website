...
const LatestReview: React.FC<{
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const fetchReview = async () => {
            ...
            setReview([{ ...data, date: Date.now() as never }, ...review]);
        };
        fetchReview()
                ...
                {user !== null && (
                    <div className="mt-4">
                        ...
                        <div className="d-flex align-items-start mb-3">
                            <div className="star-rating d-flex" style={{ cursor: "pointer", marginRight: "1rem" }}>
                                {Array.from({ length: 5 }, (_, index) => (
                                    <div key={index} onMouseEnter={() => setHoveredRating(index + 1)} onClick={() => setRating(index + 1)}>
                                        {hoveredRating !== null && hoveredRating > index ? <Star size={20} /> : <StarBlank size={20} />}
                                    </div>
                                ))}
                            </div>
                            <form onSubmit={handleSubmit} className="flex-grow-1">
                                <div className="input-group">
                                    <textarea className="form-control" placeholder="Nhập đánh giá của bạn..." value={newReview} onChange={(e) => setNewReview(e.target.value)} rows={3} required />
                                    <button className="btn btn-primary" type="submit" style={{ backgroundColor: "#007bff", borderColor: "#007bff" }}>
                                        Gửi
                                    </button>
                                </div>
                            </form>
