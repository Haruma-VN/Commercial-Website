...
const ManageUser = () => {
    ...
    const exchanger = (role: string): string => {
        const data = {
            user: "Người dùng",
    ...
    const updateUser = async (user: User) => {
        ...
        makeRole(user);
        const fetchUsers = async () => {
            console.log(JSON.stringify({ ...user, password: password || user.password }));
            const response = await fetch("http://localhost:3308/api/v1/user", {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
            ...
            <div className="d-flex justify-content-between align-items-center my-4">
                ...
                <div>
                    <button
                        ...
                        onClick={() => {
                            setPassword("");
                            onChange({ email: "", userDetail: { name: "", role: "" } } as User);
                        }}
                    >
                        Thêm
                    </button>
                ...
                <tbody>
                        ...
                        {style === changeDataStyle ? (
                            <form>
                                <div className="mb-3">
                                    ...
                                    <input type="email" className="form-control" name="email" onChange={handleInputChange} value={currentUser?.email} />
                                </div>
                                <div className="mb-3">
                                    <label className="form-label">Tên</label>
                                    <input type="text" className="form-control" name="name" onChange={handleInputChange} value={currentUser?.userDetail.name} />
                                </div>
                                <div className="mb-3">
                                    <label className="form-label">Mật khẩu</label>
                                    <input type="password" className="form-control" name="password" onChange={handleInputChange} value={password} placeholder="Đặt mật khẩu" />
                                </div>
                                <div className="mb-3">
                                    <label className="form-label">Vai trò</label>
                                    <select className="form-control" name="role" onChange={handleInputChange} value={currentUser?.userDetail.role}>
                                        <option value="admin">{exchanger("admin")}</option>
                                        <option value="user">{exchanger("user")}</option>
                                    </select>
                                </div>
                                <button onClick={() => (currentUser?.id ? updateUser(currentUser!) : addUser(currentUser!))} type="button" className="btn btn-primary my-custom-button">
                                    Lưu
                                </button>
                            ...
                            <div className="text-center p-4">
                                ...
                                <div>
                                    <button onClick={() => deleteUser(currentUser!)} type="button" className="btn btn-danger my-custom-button me-3 px-4" style={{ fontWeight: "bold" }}>
                                        Xoá
                                    </button>
                    ...
                    {users.map((user) => (
                        <tr key={user.id}>
                            ...
                            <td>{exchanger(user.userDetail.role)}</td>
                            <td>
                                <button
                                ...
                                <button className="btn btn-danger my-custom-button" onClick={() => onDelete(user)}>
                                    Xoá
                                </button>
                            </td>
                        </tr>
